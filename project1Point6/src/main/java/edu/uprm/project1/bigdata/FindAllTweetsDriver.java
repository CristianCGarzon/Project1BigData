 ...
 */
public class FindAllTweetsDriver {

    public static void main(String[] args) throws Exception{
        String nameFolder = "Problem6";
        String nameCSVFile = "AllTweets";
        String columns = "idUser, information";
        String programDescription = "Count and Find all messages posted by each user";
        ...
        job.setJarByClass(FindAllTweetsDriver.class);
        job.setJobName("Job keywords tweet counter");

        FileOutputFormat.setOutputPath(job, new Path(args[1] + "/" + nameFolder));
        FileInputFormat.addInputPath(job, path);

        job.setMapperClass(FindAllTweetsMapper.class);
        job.setReducerClass(FindAllTweetsReducer.class);
