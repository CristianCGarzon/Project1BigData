...
public class CountWordsDriver {
    ...
    public static void main(String[] args) throws Exception{
        String nameFolder = "Problem1";
        String nameCSVFile = "KeywordsAndValues";
        ...
        if(args.length == 2 && (args[0].matches(".*hdfs.*") || args[1].matches(".*hdfs.*"))){
            System.err.println("Usage fo HDFS file: Count Keywords in Tweet <input path> <output path> <output path in HDFS> <HADOOP_HOME path>");
            System.exit(-1);
        } else if(args.length != 2 && args.length != 4){
            System.err.println("Usage for HDFS File: Count Keywords in Tweet <input path> <output path> <output path in HDFS> <HADOOP_HOME path>");
            System.err.println("Usage for local File: Count Keywords in Tweet <input path> <output path>");
            System.exit(-1);
        }
        ...
        job.setJarByClass(CountWordsDriver.class);
        job.setJobName("Job specific words tweet counter");
        ...
        System.out.println("Job finish correctly. Your data is avaliable in: " + args[1] + "/" + nameFolder);
    }
}
